...
use ic_siwn::{
    ...
    NearAccountId, SignedDelegation,
};
use serde_bytes::ByteBuf;

use crate::state::{LABEL_ASSETS, LABEL_SIG, STATE};
...
fn siwn_get_delegation(
    ...
    let certificate =
        data_certificate().expect("siwn_get_delegation must be called using a query call");
    ...
    STATE.with(|s| {
        ...
        // Generate a unique seed based on the user's Near account_id.
